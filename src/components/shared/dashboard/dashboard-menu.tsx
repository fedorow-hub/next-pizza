...
export const DashboardMenu: React.FC<Props> = ({ className }) => {
  ...
  return (
    <nav className={cn('grid items-start px-4 font-medium text-lg', className)}>
      {items.map((item) => (
        <Link
          key={item.text}
          className={cn(
            'flex gap-3 rounded-[8px] px-3 py-2 mb-2 text-gray-500 transition-all hover:text-gray-900',
            {
